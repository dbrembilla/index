...
#define NBBUFF 10000
	...
	public:
		...
		mphf(size_t n, Range const &input_range, int num_thread = 1, double gamma = 2.0, bool progress = true, float perc_elem_loaded = 0.03) : _gamma(gamma), _hash_domain(size_t(ceil(double(n) * gamma))), _nelem(n), _num_thread(num_thread), _percent_elem_loaded_for_fastMode(perc_elem_loaded), _withprogress(progress)
		{
			if (n == 0)
				return;
	...
	private:
		...
		uint64_t getLevel(hash_pair_t &bbhash, const elem_t &val, int *res_level, int maxlevel = 100)
		{
			int level = 0;
			uint64_t hash_raw = 0;
